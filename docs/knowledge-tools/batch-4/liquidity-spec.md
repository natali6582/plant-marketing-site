...
# לוח נזילות והתחייבויות — מפרט עמוד אחד

נתיב: /knowledge/wealth/scenario-tool/

| מזהה קלט | שדה | ברירת מחדל בדיונית | טווח | יחידה |
|---|---|---|---|---|
| balance | מזומן זמין בתחילת הלוח | 10000 | 0–1000000000 | ₪ |
| months | מספר חודשים | 6 | 1–120 | — |

לכל חודש: inflow ו־outflow, בין 0 למיליארד ₪; ברירות המחדל המלאות ב־examples.json. ממשק 1–120 חודשים. פרמטר flows בקישור: מערך JSON של זוגות [תקבול, תשלום], עם בדיקת אורך וסוגים. אין מידע אישי בקישור.

פלט: יתרה בסוף הלוח; פער המזומן המרבי; חודשים עם פער בסוף החודש; גרף SVG חודשי; לוח מפורט של יתרת פתיחה, תקבולים, תשלומים, יתרת סגירה ופער.

הנחות: חשבון חודשי בלבד: יתרת פתיחה + תקבולים − תשלומים = יתרת סגירה. כל הסכומים בשקלים, ללא המרת מטבע, תשואה, ריבית, אינפלציה, מס או עמלות מחושבים. עלויות ידועות אפשר להזין כתשלומים. יתרה שלילית נגררת ואינה מקור אשראי. הבדיקה היא בסוף כל חודש ואינה מזהה בהכרח פער בתוך החודש. תזמון התקבולים הוא הנחת המשתמש.
