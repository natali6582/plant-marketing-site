...
# כמה צריך להפקיד כדי להגיע ליעד? — מפרט עמוד אחד

נתיב: /knowledge/planners/scenario-tool/

| מזהה קלט | שדה | ברירת מחדל בדיונית | טווח | יחידה |
|---|---|---|---|---|
| balance | יתרה התחלתית | 50000 | 0–1000000000 | ₪ |
| years | תקופת החישוב בשנים | 15 | 0–100 | — |
| return | תשואה שנתית נומינלית משוערת | 4 | -99–100 | % |
| fee | דמי ניהול מצבירה, לשנה | 0.5 | 0–100 | % |
| target | יעד נומינלי בסוף התקופה | 1000000 | 0–1000000000 | ₪ |
| returnB | תשואה שנתית בתרחיש ב׳ | 2 | -99–100 | % |

האופק הוא מספר שלם של חודשים, עד 1,200. הפסקה חייבת להיות בתוך האופק. יעד דורש אופק חיובי אם אינו ממומן כבר.

פלט: הפקדה חודשית נדרשת — תרחיש א׳; הפקדה חודשית נדרשת — תרחיש ב׳; יתרה סופית — תרחיש א׳; יתרה סופית — תרחיש ב׳; גרף SVG חודשי.

הנחות: תשואה קבועה בכל תרחיש; שיעור שנתי מומר לחודשי לפי (1 + r)^(1/12) − 1. דמי ניהול מצבירה נגבים אחרי התשואה בשיעור שנתי חלקי 12. היעד וההפקדות נומינליים; אין אינפלציה, מס, דמי ניהול מהפקדה או משיכות. ההפקדה מחושבת במנוע החודשי, מעוגלת כלפי מעלה לאגורה, ואז מורצת שוב להצגת היתרה.
