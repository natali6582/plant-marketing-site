...
> **Merge outcome (31.08.2026).** This plan was written against the
> multi-audience `main`; the work was then merged into the single-audience
> rebrand (`design/01-typography`), where four items changed: persona tabs
> dropped (audiences retired), heading weight-contrast dropped (the typography
> contract pins h1/h2 at Noto Serif 700), the marquee now runs the four
> approved /product/ advantages instead of the retired AI chips, and the hero
> ghost button is deferred (its destination was the unlinked /webinar/). The
> measured values below are unchanged and remain the source of truth.
